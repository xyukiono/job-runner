...
parser.add_argument('--timeout', type=float, default=0,
                        help='timeout(hours) to kill the current job')
...
def update_history(HISTORY, HISTORY_LOCK, job, prev_dir, prev_time, next_dir, next_time, status):
    ...
    if find_idx >= 0:
        ...
        history.append('{} {} {} {} {} {} [{}]'.format(next_time, job, next_dir, status, hostname, gpu_id, prev_time))
    else:
...
def run_jobs(config):
    ...
    for i, job in enumerate(jobfiles):
        try:
            ...
            if config.timeout > 0:
                timeout_sec = int(config.timeout * 3600)
                args = ['timeout', str(timeout_sec), 'sh', os.path.join(QUEUE_DIR, job)]
            else:
                args = ['sh', os.path.join(QUEUE_DIR, job)]
